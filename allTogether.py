...
T = Telescope(47.33982, 8.111203, 540.0/1000, 270, 90, 3, 5)
data = Data(url, path)

while True:
    data.downlad()
    P = Planes(path)
    checks = T.check_aircraft(P.pos)
    for i in range(0, len(checks)):
        if not checks[i]:
            print('ERROR: Aircraft', i, 'through the main beam\n\nSWITCH OFF THE DEVICE')
            break
        if i == 0:
            print(datetime.datetime.now(),':\nNo aircraft through the beam\n')
    time.sleep(2)
